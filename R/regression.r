...
"sm.regression" <- function(x, y, h, design.mat = NA, model = "none",
   ...
   if(!all(is.na(group))) 
      return(sm::sm.ancova(x, y, group, h, model, weights=weights,...))
...
"smplot.regression" <- function (x, y, design.mat, h, r, model, weights, rawdata = list(),
    ...
    if (!any(is.na(r$x))) {
        if (opt$band) {
            ...
            upper <- pmin(pmax(upper, graphics::par()$usr[3]), graphics::par()$usr[4])
            lower <- r$model.y - 2 * r$se
            lower <- pmin(pmax(lower, graphics::par()$usr[3]), graphics::par()$usr[4])
            graphics::polygon(c(r$eval.points, rev(r$eval.points)), c(lower, rev(upper)),
                    col = 0, border = 0)
            }
        if (opt$se | (opt$display %in% "se")) {
            ...
            lower <- pmin(pmax(lower, graphics::par()$usr[3]), graphics::par()$usr[4])
            graphics::lines(r$eval.points, upper, lty = 3, col = 0)
            graphics::lines(r$eval.points, lower, lty = 3, col = 0)
            }
        graphics::lines(r$eval.points, r$estimate, col = 0)
    }
    if (opt$band) {
        ...
        lower <- pmin(pmax(lower, graphics::par()$usr[3]), graphics::par()$usr[4])
        graphics::polygon(c(rnew$eval.points, rev(rnew$eval.points)), c(lower,
            rev(upper)), col = opt$col.band, border = 0)
    }
    graphics::lines(rnew$eval.points, rnew$estimate, lty = opt$lty, col = opt$col, lwd = opt$lwd)
    if ((model == "none") & (opt$se | (opt$display %in% "se"))) {
        upper <- rnew$estimate + 2 * rnew$se
        upper <- pmin(pmax(upper, graphics::par()$usr[3]), graphics::par()$usr[4])
        lower <- rnew$estimate - 2 * rnew$se
        lower <- pmin(pmax(lower, graphics::par()$usr[3]), graphics::par()$usr[4])
        graphics::lines(rnew$eval.points, upper, lty = 3, col = opt$col)
        graphics::lines(rnew$eval.points, lower, lty = 3, col = opt$col)
    }
    if (!opt$add)
        graphics::points(rawdata$x, rawdata$y, col = opt$col.points, pch = opt$pch,
               cex = opt$cex)
    graphics::box(col = 1, lty = 1)
    rnew
}
    ...
    if (!opt$eval.grid) {
           ...
           if (model == "none") {
              opt$col <- opt$col.palette[cut(c(se), length(opt$col.palette), labels = FALSE)]
           	  ...
           	  if (length(opt$col.palette) != length(opt$se.breaks) + 1)
           	     opt$col.palette <- 
           	                rev(grDevices::rainbow(length(opt$se.breaks) + 1, start = 0/6, end = 4/6))
        ...
        if (opt$display %in% "image") {
           ...
           if (length(opt$col) == 1) {
            ...
            graphics::persp(x1grid, x2grid, est,
...
"sm.regression.eval.2d" <- function (x, y, h, model, eval.points, 
    ...
    if (hull) {
        ...
        hull.points <- hull.points[grDevices::chull(hull.points), ]
        nh <- nrow(hull.points)
...
"sm.sigma2" <- function(x, y, 
  ...
  # else {
    ...
    D  <- sqrt(D / stats::var(X[,1]) + ((d - t(d))^2) / stats::var(X[,2]))
    nn <- function(x) {sort(x)[5]}
    hw <- apply(D, 1, nn) / 2
    hh <- c(sqrt(stats::var(X[,1])), sqrt(stats::var(X[,2])))
  ...
  if (model == "constant") {
    ...
    if (!(display == "none")) {
      ...
      graphics::contour(surface$eval.points[,1], surface$eval.points[,2],
    			surface$estimate)
      }
  ...
  if (ci) {
    ...
    bb <- (8 * k2^3) / k3^2
    cc <- k1 - aa * bb
    q  <- stats::qchisq(c(0.025, 0.975), bb)
...
"sm.sigweight" <- function(x, weights) {
  ...
  if (is.matrix(x)) {
    ...
    D  <- sqrt(D / stats::var(x1) + ((d-t(d))^2) / stats::var(x2))
    nn <- function(x) {sort(x)[5]}
    hw <- apply(D, 1, nn)/2
    h  <- c(sqrt(stats::var(x1)), sqrt(stats::var(x2)))
...
"sig.trace" <- function (expn, hvec, ...) {
    ...
    if (!(opt$display == "none")) {
        ...
        if (max(pvec) >= 0.05)
            graphics::lines(range(hvec), c(0.05, 0.05), lty = 2)
        }
