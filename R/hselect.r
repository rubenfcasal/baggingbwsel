"h.select" <- function(x, y = NA, lower, upper, weights = NA, group = NA, ...) {
  ...
  sd <- sqrt(diag(as.matrix(stats::var(x))))
  if (ndim==1)
     start <- sd / 2
  else
...
"cv" <- function (x, h, ...) {
    ...
    if (!isMatrix(x)) {
        ...
        hcvff <- sum(stats::dnorm(0, mean = 0, sd = sqrt(2) * h * opt$h.weights))/(n *
            (n - 1))
    ...
    if (isMatrix(x)) {
        x1 <- x[, 1]
        x2 <- x[, 2]
        h1 <- h * sqrt(stats::var(x1))
        h2 <- h * sqrt(stats::var(x2))
        n <- length(x1)
        replace.na(opt, h.weights, rep(1, n))
        hcvff <- sum(stats::dnorm(0, mean = 0, sd = sqrt(2) * h1 * opt$h.weights) *
                       stats::dnorm(0, mean = 0, sd = sqrt(2) * h2 * opt$h.weights))/(n *
            (n - 1))
...
"hcv" <- function (x, y = NA, hstart = NA, hend = NA, ...) {
    ...
    if (is.na(hstart)) {
        if (ndim == 1) hstart <- hnorm(x)/10
            ...
            hstart <- hnorm(x[, 1]/sqrt(stats::var(x[, 1])))/10
        else hstart <- hnorm(x[, 1]/sqrt(stats::var(x[, 1])))/4
        }
    if (is.na(hend)) {
        if (ndim == 1)
            hend <- hnorm(x) * 2
        else hend <- hnorm(x[, 1]/sqrt(stats::var(x[, 1]))) * 2
        }
        ...
        if (ndim == 2)
            for (i in 1:ngrid) {
                cvgrid[i] <- sum((y - sm.weight2(x, x,
                        exp(hgrid[i] * c(sqrt(stats::var(x[, 1])), sqrt(stats::var(x[, 2])))),
                        cross = TRUE, options = list(h.weights = h.weights)) %*% y)^2)
                }
        }
    if (any(is.na(cvgrid))) {
    ...
    if (!(display == "none")) {
        if (!opt$add) {
            ...
            if (display == "log")
                graphics::lines(hgrid, cvgrid)
            else graphics::lines(exp(hgrid), cvgrid)
            }
        }
    if (ind == 1 | ind == ngrid) {
        cat("\n")
    ...
    h <- exp(-bb/(2 * aa))
    if (ndim == 1) result <- h
    else result <- c(h * sqrt(stats::var(x[, 1])), h * sqrt(stats::var(x[, 2])))
    result
    }
...
"sj" <- function (x, h) {
    phi6 <- function(x) (x^6 - 15 * x^4 + 45 * x^2 - 15) * stats::dnorm(x)
    phi4 <- function(x) (x^4 - 6 * x^2 + 3) * stats::dnorm(x)
    n <- length(x)
    lambda <- stats::quantile(x, 0.75) - stats::quantile(x, 0.25)
    a <- 0.92 * lambda * n^(-1/7)
    ...
    result <- (stats::dnorm(0, sd = sqrt(2))/(n * abs(sdalpha2)))^0.2 - h
...
"cv.crit.dens" <- function(log.h, data, structure.2d, opt) {
   ...
   if (!is.matrix(x)) {
        hcvff <- sum(freq * stats::dnorm(0, 0, sqrt(2)*h*h.weights))/(n*(n-1))
   ...
   if (is.matrix(x)) {
    ...
    h1 <- h[1]
    h2 <- h[2]
    hcvff <- sum(freq * stats::dnorm(0, 0, sqrt(2) * h1 * h.weights) *
                   stats::dnorm(0, 0, sqrt(2) * h2 * h.weights))/(n*(n-1))
